...
const createUrl = async function (req, res) {
  try {
    ...
    if (!isValidRequestBody(requestBody)) {
      res.status(400).send({ status: false, msg: "provide long url  details" })
      return
    };
    const longUrl = requestBody.longUrl.trim().toLowerCase();     //2

    let regex = /(:?^((https|http|HTTP|HTTPS){1}:\/\/)(([w]{3})[\.]{1})?([a-zA-Z0-9]{1,}[\.])[\w]*((\/){1}([\w@?^=%&amp;~+#-_.]+))*)$/;
    if (!regex.test(longUrl)) {
      return res.status(400).send({ status: false, message: `${longUrl} this URl is not a valid Url` })
    };
    if (longUrl) {
